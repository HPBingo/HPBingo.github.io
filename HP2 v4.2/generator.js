...
bingoGenerator = function(bingoListR, opts) {
    ...
    if (true) {
        ...
        for (var i = 1; i <= 25; i++) {
            ...
            var possibleList = [];
            for (var j = 0; j < 4; j++) {
                if (j >= 3 && possibleList.length < 1) {
                    j--;
                }
                var d = 0;
                do {
                    d = parseInt(getDifficulty + boxMullerRand() * 4);
                } while (d <= 0 || d > 25);
                var goal = bingoList[d][parseInt(Math.random() * bingoList[d].length)];
                if (used.indexOf(goal) != -1 || possibleList.indexOf(goal) != -1) {
                    continue;
                }
                possibleList.push(goal);
            }
            var RNG = Math.floor(possibleList.length * Math.random());
            if (RNG == possibleList.length) {
                RNG--;
            };
            var j = 0,
                synergy = 0,
                ...
                minSynObj = null;
            do {
                currentObj = possibleList[(j + RNG) % possibleList.length];
            ...
            name = name.replaceAll(/\{([\d,]+)\}/g, (match, p1, offset, string) => {
                var parts = p1.split(",");
                var v = parseInt(Math.random() * parts.length);
                return parts[v];
            });
        ...
        if (MODE == "lockout") {
            ...
            for (var i = 0; i < lockoutGoals; i++) {
                ...
                var goal;
                do {
                    goal = bingoListR["lockout"][parseInt(Math.random() * bingoListR["lockout"].length)]
                } while (usedLockout.indexOf(goal) != -1);
                usedLockout.push(goal);
                bingoBoard[b].synergy = 0;
                bingoBoard[b].types = {};
