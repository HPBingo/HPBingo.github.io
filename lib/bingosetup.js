...
document.querySelectorAll('#bingo tr td:not(.popout):not(#board-popout):not(#bingo-info):not(#bingosync), #selected td').forEach(el => el.addEventListener('click', () => {
...
boardInfo.innerHTML = `
HP Bingo <strong>${bingoOpts.version}</strong>&emsp;
...
document.querySelectorAll('#bingosync').forEach(el => el.addEventListener('click', () => {
  let t = `[{"name": "`;
  let n = `"},{"name": "`;
  for (let i = 1; i<26; i++){t+=document.getElementById("slot"+i).innerHTML;if (i<25){t+=n;}}  
  t +=`"}]`;
  alert(t);
}));
...
if (newCards) {
  if (newCards.attributes.lockout) {
    newCards.innerHTML = `
...
<a class="newcard" href="?mode=lockout&seed=${Math.ceil(999999 * Math.random()).toString()}">Lockout Card</a>
`;
  } else {
    newCards.innerHTML = `
<a class="newcard" href="?mode=normal&seed=${Math.ceil(999999 * Math.random()).toString()}">Normal Card</a>
